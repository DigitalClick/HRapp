...
try {
  ...
  alert('speak.js warning: no worker support');
}
...
function speak(text, args) {
  ...
  function handleWav(wav) {
    ...
    if (PROFILE) alert('speak.js: wav processing took ' + (Date.now()-startTime).toFixed(2) + ' ms');
  }
  ...
  if (args && args.noWorker) {
    ...
    if (PROFILE) alert('speak.js: processing took ' + (Date.now() - startTime).toFixed(2) + ' ms');
    handleWav(wav);
  } else {
    // Call the worker, which will return a wav that we then play
    var startTime = Date.now();
    speakWorker.onmessage = function(event) {
        if (PROFILE) alert('speak.js: worker processing took ' + (Date.now() - startTime).toFixed(2) + ' ms');
